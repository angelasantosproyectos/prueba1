...
async function loadChats() {
  ...
  try {
    // Sin orderBy para evitar necesitar índice compuesto — ordenamos en cliente
    ...
    // Filtrar docs con participantes válidos y ordenar por ultimoMensajeAt desc en cliente
    let docs = snap.docs.filter(d => {
      const p = d.data().participantes;
      return Array.isArray(p) && p.length === 2 && p.every(x => x && x.trim() !== '');
    });
    docs.sort((a, b) => {
      const ta = a.data().ultimoMensajeAt;
      const tb = b.data().ultimoMensajeAt;
      const va = ta && ta.toDate ? ta.toDate().getTime() : 0;
      const vb = tb && tb.toDate ? tb.toDate().getTime() : 0;
      return vb - va;
    });
    ...
    for (let i = 0; i < docs.length; i++) {
      ...
      const hora = chat.ultimoMensajeAt && chat.ultimoMensajeAt.toDate
...
async function abrirConversacion(chatId, otroUid, otroData) {
  ...
  // Asegurarse de que el doc del chat tiene todos los campos necesarios
  try {
    const ref = db.collection('chats').doc(chatId);
    const snap = await ref.get();
    if (!snap.exists) {
      ...
      // Reparar campos corruptos o faltantes
      const data = snap.data();
      const fixes = {};
      if (!data.noLeidos) fixes.noLeidos = { [currentUser.uid]: 0, [otroUid]: 0 };
      if (!data.ultimoMensaje && data.ultimoMensaje !== '') fixes.ultimoMensaje = '';
